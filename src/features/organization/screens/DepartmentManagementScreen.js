...
import { departmentsAPI, unwrapResponse } from '../../../core/api/api';
...
export default function DepartmentManagementScreen() {
  ...
  const createDepartment = async () => {
    ...
    if (parsedManagerId === undefined) {
      Alert.alert('Mã quản lý không hợp lệ', 'Mã quản lý phải là số nguyên dương.');
      return;
    }
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <View style={styles.headerWrap}>
            <Text style={[styles.title, { color: colors.text }]}>Phòng ban</Text>
        ...
        details={
          selectedDepartment
            ? [
                ...
                { label: 'Mã quản lý', value: String(selectedDepartment.manager || '--') },
              ]
            : []
        }
      />
