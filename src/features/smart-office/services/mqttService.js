...
class SmartOfficeMqttService {
  ...
  connect() {
    if (!this.canControl) {
      return Promise.reject(new Error('Cần chấm công vào để điều khiển văn phòng thông minh'));
    }
  ...
  publishDeviceState(deviceKey, state) {
    if (!this.canControl) {
      throw new Error('Cần chấm công vào để điều khiển văn phòng thông minh');
    }

    const topic = SMART_OFFICE_TOPICS[deviceKey];
    if (!topic) {
      throw new Error(`Thiết bị văn phòng thông minh không xác định: ${deviceKey}`);
    }
