...
import { overtimeAPI, unwrapResponse } from '../services/api';
    ...
    <ScreenContainer scroll={false}>
      ...
      {!isManager ? (
        <AnimatedCard style={[styles.formCard, { backgroundColor: colors.card, borderColor: colors.border, borderRadius: radius.md }]}>
          <Text style={[styles.sectionTitle, { color: colors.text }]}>Gui yeu cau moi</Text>
          <FloatingField
            label="Ngày tăng ca"
            value={form.date}
          ...
          />
          {otRate ? (
            <Text style={[styles.hint, { color: colors.textMuted }]}>Loai ngay: {otRate.label} | He so OT: x{otRate.rate}</Text>
          ) : null}
          <PrimaryButton
