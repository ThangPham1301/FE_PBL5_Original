...
import { reportsAPI, unwrapResponse } from '../services/api';
...
export default function ReportsScreen() {
  ...
  const exportReport = async () => {
    if (!isPositiveInteger(filters.month) || !isPositiveInteger(filters.year)) {
      Alert.alert('Thiếu dữ liệu', 'Vui lòng nhập tháng và năm hợp lệ trước khi export.');
      return;
    }
    ...
    try {
      ...
      if (!token) {
        Alert.alert('Phiên đăng nhập hết hạn', 'Vui lòng đăng nhập lại để export báo cáo.');
        return;
      }
      ...
      if (canShare) {
        ...
        Alert.alert('Export thành công', `Đã tải file về: ${result.uri}`);
      }
    } catch (error) {
      Alert.alert('Lỗi', error.message || 'Không thể export báo cáo.');
    } finally {
      setExporting(false);
    }
  };
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <View style={styles.headerWrap}>
            ...
            />
            <PrimaryButton
              title={exporting ? 'Đang export...' : 'Export Excel'}
              variant="secondary"
